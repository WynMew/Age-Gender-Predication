...
def drawBoxes(im, boxes, points):
    ...
    for i in range(x1.shape[0]):
        cv2.rectangle(im, (int(x1[i]), int(y1[i])), (int(x2[i]), int(y2[i])), (255,0,0), 3)
        for j in range(0,5):
            cv2.circle(im,(points[i,j],points[i,j+5]), 4, (0,0,255))
    return im
...
def detect_face_PNet(img, minsize, PNet, threshold, fastresize=False):
    ...
    for scale in scales:
        ...
        out_map = out['prob1'][0,1,:,:]
        out_reg = out['conv4-2'][0]

    return out_reg, out_map

def detect_face(img, minsize, PNet, RNet, ONet, threshold, fastresize, factor, skip=False):
...
def main():
    ...
    threshold = [0.6, 0.7, 0.7]
    factor = 0.709   
    caffe_model_path = '/home/HDD2/lupingping/MTCNNCaffe-test/models/detection'
    PNet = caffe.Net(caffe_model_path+"/det1.prototxt", caffe_model_path+"/det1.caffemodel", caffe.TEST)
    RNet = caffe.Net(caffe_model_path+"/det2.prototxt", caffe_model_path+"/det2.caffemodel", caffe.TEST)
    ONet = caffe.Net(caffe_model_path+"/det3.prototxt", caffe_model_path+"/det3.caffemodel", caffe.TEST)
    ...
    dataset = 'imdbAge'
    Path = '/home/HDD4/Database/imdbAge'
    Image_list='/home/HDD4/Database/imdbAge/imdbagelist'
    Save_root = '/home/HDD4/Database/imdbAge/imdbMTCNN'
    Save_list = '/home/HDD4/Database/imdbAge/imdbMTCNN.txt'
    ...
    for m in it:
        ...
        Save_Path = Save_root +"/" +file[3]

        img = cv2.imread(Image_Path)
        height = img.shape[0]
        width = img.shape[1]
        restrict_size = 1500.0
        if max(height,width)>restrict_size:
            scale = restrict_size/max(height,width)
            img = cv2.resize(img,(np.int32(width*scale),np.int32(height*scale)), 0.0, 0.0, interpolation=cv2.INTER_CUBIC)
        ...
        boundingboxes, points = detect_face(img_matlab, minsize, PNet, RNet, ONet, threshold, False, factor)
        print(m)
        #print(boundingboxes.size)

        if boundingboxes.size == 5:
            print(boundingboxes)
            img = img[max(boundingboxes[0][1],0):min(boundingboxes[0][3],height), max(boundingboxes[0][0],0):min(boundingboxes[0][2],width)]
