...
dataLoaderTrain = data.DataLoader(datasetTrain, batch_size=24, shuffle=True, num_workers=1)
datasetTest = MyDataSet(filelist ='/home/miaoqianwen/AgePre/DataAgeVal',
            transform=transforms.Compose([
                ToTensorDict(),
                NormalizeImageDict(['image'])
            ]))
dataLoaderTest = data.DataLoader(datasetTest, batch_size=24, shuffle=True, num_workers=1)

def add(x):
    with open('TrainAgePreReResNet256Log',"a+") as outfile:
        outfile.write(x + "\n")
...
for epoch in range(start_epoch, end_epoch + 1):
    ...
    #checkpoint_name = os.path.join('imdbwikiAgePreResNet256_CrossEntloss.pth.tar')
    checkpoint_name = os.path.join('imdbwikiAgePreReResNet256_CrossEntloss.pth.tar')
    save_checkpoint({
        'epoch': epoch + 1,
